...
class Station(Location):
    ...
    def get_transit_type(self) -> str:
        ...
        return "/".join([key for key, is_transit in has_transit_types.items() if is_transit is True]).strip()
...
def retrieve_stns(stn_file: str,
                  col_name: str, col_lat: str, col_lon: str, col_code: str,
                  region: str) -> list:
    stn_list = []
...
def get_stn_df() -> pd.DataFrame:
    stn_listed = [
        [stn.name, stn.coords[0], stn.coords[1], stn.get_transit_type()]
        for stn in station_list]
    
    stn_df = pd.DataFrame(stn_listed,
...
def get_hotspot_df(hotspots_list: list[Hotspot]):
    hspt_listed = [
        [hspt.name, hspt.coords[0], hspt.coords[1]]
        for hspt in hotspots_list
    ]
    
    hspt_df = pd.DataFrame(hspt_listed,
...
def gen_location(loc_name: str, coords: tuple[float, float], region: str, predefined_hotspots_store: list[dict], target_hotspot_store: list[Hotspot]) -> Hotspot:
    ...
    if nearest_hspt and dist_ratio <= 1:
        logger.debug(FOUND_HSPT_START
                     + "distance ratio: " + str(dist_ratio)
                     + ", name: " + loc_name
                     + " -> " + nearest_hspt.name)
        return nearest_hspt
    # Similar name check
    ...
    if hotspot_params:
        ...
        if region_list[region].metro_data: # Nearest station + cleaned name check
            ...
            if ((not cleaned_loc_name)
                or cleaned_loc_name.startswith("Singapore")
                or ("Auto selected" in cleaned_loc_name)
            ):
                cleaned_loc_name = stn_out.get_disp_str()
